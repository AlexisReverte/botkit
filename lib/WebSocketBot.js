...
function WebSocketBot(configuration) {
    ...
    websocket_botkit.defineBot(function (botkit, config) {
        ...
        bot.send = function (message, cb) {
            ...
            if (chats[message.user]) {
                botkit.debug('*** SEND VIA WS', message);
                chats[message.user].ws.send(JSON.stringify(message));
            } else {
                botkit.debug('*** Send to callback because there is no local socket opened');
    ...
    websocket_botkit.setupWebserver = function (port, cb, app) {
        if (!port) {
        ...
        if (!app) {
            ...
            websocket_botkit.webserver = app;
